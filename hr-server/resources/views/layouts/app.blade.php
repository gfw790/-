...
<head>
    ...
    <title>@yield('title', '인사관리 시스템')</title>
...
<body>
    <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="{{ route('departments.index') }}" class="nav-link {{ request()->routeIs('departments.index', 'departments.create', 'departments.edit') ? 'active' : '' }}">
                <i class="fas fa-sitemap"></i> 부서 관리
            </a>
            <a href="{{ route('departments.tree') }}" class="nav-link {{ request()->routeIs('departments.tree') ? 'active' : '' }}">
                <i class="fas fa-project-diagram"></i> 조직도
            </a>
        </nav>
        <div style="padding:16px 12px; border-top:1px solid rgba(255,255,255,.08);">
            <a href="{{ route('users.index') }}" style="font-size:12px; color:#94a3b8; margin-bottom:8px; padding:4px; border-radius:6px; display:block; text-decoration:none; transition:all .15s;"
               ...
               onmouseout="this.style.background=''; this.style.color='#94a3b8';">
                <i class="fas fa-user-circle" style="margin-right:6px;"></i>{{ auth()->user()->name ?? auth()->user()->username }}
            </a>
