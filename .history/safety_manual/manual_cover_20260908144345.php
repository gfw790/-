...
$manualDataPath = __DIR__ . '/data.json';
if (is_file($manualDataPath)) {
    $manualData = json_decode((string)file_get_contents($manualDataPath), true);
    $manualHtml = (string)($manualData['current']['content_html'] ?? '');
    if (preg_match('/현대기전[-‐‑–—]중대[-‐‑–—]?[A-Za-z0-9가-힣-]*/u', $manualHtml, $documentMatch)) {
        $coverData['document_number'] = trim(strip_tags($documentMatch[0]));
    }
}
$message = '';
$errorMessage = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!hash_equals((string)$_SESSION['cover_csrf'], $csrf)) {
        ...
        $documentNumber = (string)($coverData['document_number'] ?? '');
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .control-options label { display: flex; align-items: center; gap: 6px; white-space: nowrap; }
    ...
    <header class="topbar">
        <div class="topbar-inner">
            ...
            <div class="toolbar">
                ...
                <div class="user-chip"><?= cover_h((string)($user['name'] ?? '5878')) ?>님</div>
            </div>
        </div>
    </header>
    <main class="paper">
        ...
        <div class="cards">
            <section class="card editor-card">
                ...
                <form method="post">
                    ...
                    <div class="field"><label for="document_number">문서번호</label><input id="document_number" name="document_number" value="<?= cover_h((string)($coverData['document_number'] ?? '')) ?>" readonly></div>
