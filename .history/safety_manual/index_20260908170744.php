        ...
        (function () {
            ...
            function buildPrintPreview() {
                ...
                function startChartPage() {
                    ...
                    // 체계도가 A4 한 장을 넘지 않도록 필요 시 축소한다 (다른 페이지에는 영향 없음).
                    var chartNaturalHeight = chartClone.getBoundingClientRect().height;
                    var chartNaturalWidth = chartClone.getBoundingClientRect().width;
                    var chartScale = 1;
                    if (availableHeight > 0 && chartNaturalHeight > availableHeight) {
                        chartScale = availableHeight / chartNaturalHeight;
                    }
                    if (chartScale < 1) {
                        chartClone.style.transform = 'scale(' + chartScale + ')';
                        chartClone.style.transformOrigin = 'top left';
                    }
                    chartDocument.style.height = Math.ceil(chartNaturalHeight * chartScale + 12) + 'px';
                    chartDocument.style.width = Math.ceil(chartNaturalWidth * chartScale / Math.max(chartScale, 0.01)) + 'px';
