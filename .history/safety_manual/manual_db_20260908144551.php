...
function safety_manual_db_save(PDO $db, string $contentHtml, array $coverData, string $editor): string
{
    ...
    $dom = new DOMDocument();
    if (@$dom->loadHTML('<?xml encoding="utf-8" ?><div id="manual-clause-root">' . $contentHtml . '</div>', LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD | LIBXML_NONET)) {
        $root = $dom->getElementById('manual-clause-root');
        if ($root instanceof DOMElement) {
            $clauseStmt = $db->prepare('INSERT INTO safety_manual_clauses (revision_key, clause_key, heading, body, updated_at) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE heading=VALUES(heading), body=VALUES(body), updated_at=VALUES(updated_at)');
            $now = date('Y-m-d H:i:s');
            foreach ($root->getElementsByTagName('h2') as $heading) {
                if (!$heading instanceof DOMElement) continue;
                $keyText = trim((string)$heading->getAttribute('id')) ?: sha1($heading->textContent);
                $body = '';
                for ($node = $heading->nextSibling; $node instanceof DOMNode; $node = $node->nextSibling) {
                    if ($node instanceof DOMElement && in_array(strtolower($node->tagName), ['h2', 'h3'], true)) break;
                    $body .= $dom->saveHTML($node);
                }
                $clauseStmt->execute([$key, $keyText, trim($heading->textContent), trim($body), $now]);
            }
            foreach ($root->getElementsByTagName('div') as $wrap) {
                if (!$wrap instanceof DOMElement || !preg_match('/(^|\s)manual-subtitle-box(\s|$)/', $wrap->getAttribute('class'))) continue;
                $cell = $wrap->getElementsByTagName('td')->item(0);
                if (!$cell instanceof DOMElement) continue;
                $headingText = trim($cell->textContent);
                if ($headingText === '') continue;
                $clauseKey = trim($cell->getAttribute('id')) ?: sha1($headingText);
                $body = '';
                for ($node = $wrap->nextSibling; $node instanceof DOMNode; $node = $node->nextSibling) {
                    if ($node instanceof DOMElement && (preg_match('/(^|\s)manual-subtitle-box(\s|$)/', $node->getAttribute('class')) || in_array(strtolower($node->tagName), ['h2', 'h3'], true))) break;
                    $body .= $dom->saveHTML($node);
                }
                $clauseStmt->execute([$key, $clauseKey, $headingText, trim($body), $now]);
            }
        }
    }
    return $key;
