    ...
    <script>
        (function () {
            ...
            function prepareSafetyManualSectionAnchor() {
                ...
                var sectionEndNode = Array.prototype.find.call(documentRoot.querySelectorAll('p, h2, h3'), function (node) {
                    if (node === targetNode || (targetNode.compareDocumentPosition(node) & Node.DOCUMENT_POSITION_FOLLOWING) === 0) {
                        return false;
                    }

                    var title = String(node.textContent || '').replace(/\s+/g, ' ').trim();
                    return /^6\.\s*안전보건업무\s*주관부서/.test(title);
                });
                ...
                if (sectionEndNode) {
                    sectionEndNode.parentNode.insertBefore(policyLink, sectionEndNode);
                } else {
                    documentRoot.appendChild(policyLink);
                }
            }
            ...
            function buildPrintPreview() {
                ...
                var sampleHeaderTable = documentRoot.querySelector('.rule-table-wrap');
                ...
                var totalPages = pageSections.length;
                ...
                pageSections.forEach(function (sectionNodes, index) {
                    ...
                    if (sampleHeaderTable) {
                        var pageHdr = sampleHeaderTable.cloneNode(true);
                        // 페이지 번호 갱신 (페 이 지 칸)
                        ...
                        for (var c = 0; c < pageCells.length; c++) {
                            if (/페\s*이\s*지/u.test(pageCells[c].textContent || '') && pageCells[c + 1]) {
                                pageCells[c + 1].textContent = (index + 1) + '/' + totalPages;
                                break;
                            }
                        }
                        page.appendChild(pageHdr);
                    } else if (index === 0 && sourceHeader) {
                    ...
                    sectionNodes.forEach(function (node) {
                        // 중복 헤더 테이블 방지
                        if (sampleHeaderTable && node.classList && node.classList.contains('rule-table-wrap') && node.querySelector('td[rowspan="4"]')) {
                            return;
                        }
