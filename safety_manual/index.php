...
function safety_manual_build_cover_html(array $coverData): string
{
    ...
    $revisionCount = trim((string)($coverData['revision_count'] ?? ''));
    if ($revisionCount === '') {
        $revisionCount = '0';
    }
    ...
    return '<section class="manual-output-cover">'
        ...
        . '<tr><th>개정차수</th><td>' . h($revisionCount) . '</td></tr>'
        . '</tbody></table>'
...
function safety_manual_build_pdf_html(string $title, string $contentHtml, ?array $current = null): string
{
    ...
    return '<!DOCTYPE html>'
        ...
        . '.document p { margin: 0 0 3mm; line-height: 1.9; }'
        ...
        . '.document .manual-output-header tr:first-child td:last-child { font-size: 9pt; padding-left: 1.5mm; padding-right: 1.5mm; }'
...
function safety_manual_extract_manual_header_html(string $contentHtml): string
{
    ...
    if ($headerNode instanceof DOMElement) {
        $headerNode->setAttribute('class', trim($headerNode->getAttribute('class') . ' manual-output-header'));
    }
...
function safety_manual_apply_cover_to_header_html(string $headerHtml, array $coverData): string
{
    ...
    foreach ($cells as $cell) {
        ...
        if (str_contains($label, '개정차수')) {
            $value = trim((string)($coverData['revision_count'] ?? ''));
            if ($value === '') $value = '0';
        }
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .print-preview-page .print-chart-frame {
            width: calc(100% - 5%);
            margin-left: 5%;
            margin-right: 0;
            padding: 0;
        ...
        .print-preview-page .print-chart-frame button {
            cursor: default;
            pointer-events: none;
        }
        .print-chart-frame .chart-section-blue { position: relative; }
        .print-chart-title {
            position: absolute;
            top: 0;
            right: 0;
            width: calc(var(--chart-branch-column) * 0.77);
            height: 65.12px;
            display: flex;
            flex-direction: column;
            justify-content: center;
        ...
        .print-preview-page.print-chart-page {
            height: auto;
            min-height: 0;
        }

        .print-preview-page.print-chart-page .document {
        ...
        .print-preview-page > .rule-table-wrap .rule-table tr:first-child td:last-child {
            font-size: 9px;
            padding-left: 3px;
            padding-right: 3px;
        }
        ...
        .insert-position-dialog { max-width: 520px; }
        .insert-position-body { padding: 22px 20px; }
        .insert-position-body label { display: block; margin-bottom: 8px; color: #17315c; font-weight: 700; }
        .insert-position-body select { width: 100%; min-height: 44px; border: 1px solid #cdd9ea; border-radius: 10px; padding: 8px 10px; font: inherit; }
        ...
        .rule-table-wrap.manual-output-header tr:first-child td:last-child {
            font-size: 12px;
            padding-left: 3px;
            padding-right: 3px;
        }
        ...
        .chart-frame {
            --chart-branch-column: 244px;
            --chart-column-gap: 10px;
        ...
        .chart-head-blue {
            ...
            border-radius: 12px;
            width: calc(100% - var(--chart-branch-column) - var(--chart-column-gap));
            padding: 7px 9px;
            font-size: 11.5px;
            font-weight: 800;
            color: #0369a1;
        ...
        .chart-row-flex {
            display: grid;
            grid-template-columns: minmax(0, 1fr) var(--chart-branch-column);
            gap: var(--chart-column-gap);
            align-items: center;
            position: relative;
        }

        .chart-frame > .chart-head-orange,
        .chart-frame > .chart-head-purple,
        .chart-section-green > .chart-head-green {
            width: calc(100% - var(--chart-branch-column) - var(--chart-column-gap));
        }
        ...
        .chart-section-blue .chart-item-list {
            position: relative;
        }

        .chart-section-blue .chart-item-list::before {
            display: none;
        }

        .chart-section-blue .chart-item-list > .chart-item-btn:nth-child(3)::after,
        .chart-section-blue .chart-item-list > .chart-item-btn:nth-child(7)::after,
        .chart-section-blue .chart-item-list > .chart-item-btn:nth-child(8)::after,
        .chart-section-blue .chart-item-list > .chart-item-btn:nth-child(9)::after {
            content: '';
            position: absolute;
            right: calc(var(--chart-column-gap) * -1);
            width: var(--chart-column-gap);
            border-top: 1.5px solid #38a3b5;
        }

        .chart-section-blue .chart-item-list > .chart-item-btn:nth-child(3),
        .chart-section-blue .chart-item-list > .chart-item-btn:nth-child(7),
        .chart-section-blue .chart-item-list > .chart-item-btn:nth-child(8),
        .chart-section-blue .chart-item-list > .chart-item-btn:nth-child(9) {
            position: relative;
        }
        ...
        .chart-section-blue .chart-branch-group:nth-child(1) {
            top: 45px;
        }

        .chart-section-blue .chart-branch-group:nth-child(2) {
            top: 71px;
        }

        .chart-section-green .chart-branch-group {
            top: 26px;
        }

        .chart-branch-col::before {
            content: '';
            position: absolute;
            left: -5px;
            top: 20%;
            bottom: 20%;
            border-left: 1.5px solid #38a3b5;
        }

        .chart-section-blue .chart-branch-col::before {
            display: none;
        }

        .chart-section-green .chart-branch-col::before {
            display: none;
        }

        .chart-section-blue .chart-branch-group::before {
            display: block;
            width: calc(var(--chart-column-gap) + 1px);
            height: 0;
            margin-left: calc(var(--chart-column-gap) * -1);
            border-top: 1.5px solid #38a3b5;
            flex: 0 0 calc(var(--chart-column-gap) + 1px);
        }

        .chart-section-blue .chart-branch-group:first-child::before {
            display: none;
        }

        .chart-branch-group::before {
            content: '';
            width: 10px;
            border-top: 1.5px solid #38a3b5;
            flex: 0 0 10px;
        ...
        /* 출력 도식에서는 버튼 사이의 연결 선분을 표시하지 않는다. */
        .chart-frame .chart-branch-col::before,
        .chart-frame .chart-branch-group::before,
        .chart-frame .chart-item-list > .chart-item-btn::after {
            display: none !important;
            content: none !important;
        }

        .chart-frame .chart-connectors {
            display: none !important;
        }
        ...
        .chart-frame .chart-arrow-line {
            display: block !important;
        }

        .chart-section-blue .chart-branch-group:nth-child(1) .chart-branch-badge-orange,
        .chart-section-blue .chart-branch-group:nth-child(1) .chart-arrow-line,
        .chart-section-blue .chart-branch-group:nth-child(1) .chart-branch-desc-orange,
        .chart-section-blue .chart-branch-group:nth-child(2) .chart-branch-badge-teal,
        .chart-section-blue .chart-branch-group:nth-child(2) .chart-arrow-line,
        .chart-section-blue .chart-branch-group:nth-child(2) .chart-branch-desc-teal,
        .chart-section-green .chart-branch-group .chart-branch-badge-green,
        .chart-section-green .chart-branch-group .chart-arrow-line,
        .chart-section-green .chart-branch-group .chart-branch-desc-green {
        ...
        .chart-row-flex > *:not(.chart-connectors) {
            position: relative;
            z-index: 1;
        }

        /* 3·7·8·9 버튼과 점검 배지를 연결하는 청록색 선분 */
        .chart-frame .chart-section-blue .chart-connectors {
            display: block !important;
        }

        .chart-frame .chart-section-blue .chart-connectors path {
            stroke: #0284c7;
        }

        /* 녹색 섹션: 2·4번 버튼과 우측 오브젝트를 연결하는 녹색 선분 */
        .chart-frame .chart-section-green .chart-connectors-green {
            display: block !important;
        }

        .chart-frame .chart-section-green .chart-connectors-green path {
            stroke: #16a34a;
        }

        /* 5번 버튼과 평가·관리 배지를 연결하는 주황색 선분 */
        .chart-frame .chart-section-blue .chart-connectors path.chart-line-orange {
            stroke: #ea580c;
        }

        /* 5번 버튼 테두리를 우측 평가 오브젝트 색상으로 통일 */
        .chart-section-blue .chart-item-list > #chart-item-5 {
            border-color: #ea580c;
        }
        ...
        .chart-section-blue .chart-branch-group:nth-child(2)::before {
            display: none !important;
            content: none !important;
        }

        /* 3·7·8·9 버튼 테두리를 우측 점검 오브젝트 색상으로 통일 */
        .chart-section-blue .chart-item-list > #chart-item-3,
        .chart-section-blue .chart-item-list > #chart-item-7,
        .chart-section-blue .chart-item-list > #chart-item-8,
        .chart-section-blue .chart-item-list > #chart-item-9 {
            border-color: #0284c7;
        }
...
<body>
                    ...
                    <div class="chart-frame">
                        ...
                        <div class="chart-section-blue">
                            ...
                            <div class="chart-row-flex">
                                <svg class="chart-connectors" aria-hidden="true" preserveAspectRatio="none">
                                    <path id="chart-line-3"></path>
                                    <path id="chart-line-5" class="chart-line-orange"></path>
                                    <path id="chart-line-7"></path>
                                    <path id="chart-line-8"></path>
                                    <path id="chart-line-9"></path>
                                </svg>
                                <div class="chart-item-list">
                                    ...
                                    <button id="chart-item-7" type="button" class="chart-btn chart-item-btn" data-law-query="중대재해처벌법 시행령 제4조 제7호">7. 종사자 의견 청취 절차마련, 청취 및 개선방안 마련·이행 여부 점검</button>
                                    <button id="chart-item-8" type="button" class="chart-btn chart-item-btn" data-law-query="중대재해처벌법 시행령 제4조 제8호">8. 중대산업재해 발생 시 등 조치 매뉴얼 마련 및 조치 여부 점검</button>
                                    <button id="chart-item-9" type="button" class="chart-btn chart-item-btn" data-law-query="중대재해처벌법 시행령 제4조 제9호">9. 도급, 용역, 위탁 시 평가기준·절차 및 관리비용, 업무수행기관 관련 기준 마련·이행 여부 점검</button>
                                </div>
                                <div class="chart-branch-col">
                                    <!-- Branch for Item 5 -->
                                    <div class="chart-branch-group">
                                        <div id="chart-blue-eval" class="chart-branch-badge-orange">평가·관리<br>반기 1회 이상</div>
                                    ...
                                    <div class="chart-branch-group" style="margin-top: 14px;">
                                        <div id="chart-blue-check" class="chart-branch-badge-teal">점검 반기<br>1회 이상</div>
                                        <div class="chart-arrow-line" style="color:#0284c7;">➔</div>
                        ...
                        <div class="chart-section-green">
                            ...
                            <div class="chart-row-flex">
                                <svg class="chart-connectors chart-connectors-green" aria-hidden="true" preserveAspectRatio="none">
                                    <path id="chart-line-g2"></path>
                                    <path id="chart-line-g4"></path>
                                </svg>
                                <div class="chart-item-list">
                                    ...
                                    <button id="chart-item-g2" type="button" class="chart-btn chart-item-btn-green" data-law-query="중대재해처벌법 시행령 제5조 제2항">2. 인력 배치 및 예산 추가 편성·집행 등 의무 이행에 필요한 조치</button>
                                    ...
                                    <button id="chart-item-g4" type="button" class="chart-btn chart-item-btn-green" data-law-query="중대재해처벌법 시행령 제5조 제4항">4. 미실시 교육에 대한 이행의 지시, 예산의 확보 등 교육 실시에 필요한 조치</button>
                                </div>
                                <div class="chart-branch-col">
                                    <div class="chart-branch-group">
                                        <div id="chart-green-check" class="chart-branch-badge-green">점검 반기<br>1회 이상</div>
                                        <div class="chart-arrow-line" style="color:#16a34a;">➔</div>
                                        <button id="chart-green-desc" type="button" class="chart-btn chart-branch-desc-green" data-law-query="중대재해처벌법 시행령 제5조">
                                            2. 의무 이행 필요조치<br>
                                            4. 교육 실시 필요조치
                                        </button>
                                    </div>
    ...
    <div class="rule-edit-modal" id="insert-position-modal" aria-hidden="true">
        <div class="rule-edit-dialog insert-position-dialog" role="dialog" aria-modal="true" aria-labelledby="insert-position-title">
            <div class="rule-edit-head">
                <h3 id="insert-position-title">새 조항 위치 선택</h3>
                <button type="button" class="rule-edit-close" id="insert-position-close" aria-label="닫기">&times;</button>
            </div>
            <div class="insert-position-body">
                <label for="insert-position-select">어느 조항 뒤에 추가할까요?</label>
                <select id="insert-position-select"></select>
            </div>
            <div class="rule-edit-foot">
                <button type="button" class="rule-edit-btn" id="insert-position-cancel">취소</button>
                <button type="button" class="rule-edit-btn primary" id="insert-position-confirm">선택완료</button>
            </div>
        </div>
    </div>
    ...
    <script>
        (function () {
            ...
            function isArticleHeading(node) {
                ...
                if (node.classList && node.classList.contains('manual-subtitle-box')) {
                    return true;
                }

                if (node.tagName === 'H2') {
                    return !node.classList.contains('manual-chapter-title');
                }
            ...
            function getArticleHeadingNodes() {
                ...
                return Array.prototype.filter.call(documentRoot.querySelectorAll('h2, h3, .manual-subtitle-box'), function (node) {
            ...
            // 체계도 도식 내 버튼 클릭 이벤트
            function updateChartConnectors(root) {
                root = root || document;
                var row = root.querySelector('.chart-section-blue .chart-row-flex');
                var svg = root.querySelector('.chart-section-blue .chart-connectors');
                var target = root.querySelector('#chart-blue-check');
                if (!row || !svg || !target) return;

                var rowRect = row.getBoundingClientRect();
                var targetRect = target.getBoundingClientRect();
                var width = Math.max(1, Math.ceil(rowRect.width));
                var height = Math.max(1, Math.ceil(rowRect.height));
                svg.setAttribute('viewBox', '0 0 ' + width + ' ' + height);

                var sourceRects = {};
                [3, 7, 8, 9].forEach(function (number) {
                    var source = root.querySelector('#chart-item-' + number);
                    if (!source) return;

                    var sourceRect = source.getBoundingClientRect();
                    sourceRects[number] = {
                        x: sourceRect.right - rowRect.left,
                        y: sourceRect.top + sourceRect.height / 2 - rowRect.top,
                    };
                });

                var points = Object.keys(sourceRects).map(function (number) { return sourceRects[number]; });
                if (points.length === 0) return;
                var trunkX = Math.max.apply(null, points.map(function (point) { return point.x; })) + 8;
                var targetX = targetRect.left - rowRect.left;
                var targetY = targetRect.top + targetRect.height / 2 - rowRect.top;
                var source3 = sourceRects[3];
                var source7 = sourceRects[7];
                var source8 = sourceRects[8];
                var source9 = sourceRects[9];

                if (source3) {
                    root.querySelector('#chart-line-3').setAttribute('d', 'M ' + source3.x + ' ' + source3.y + ' H ' + trunkX + ' V ' + (source9 ? source9.y : source3.y));
                }
                if (source7) {
                    root.querySelector('#chart-line-7').setAttribute('d', 'M ' + source7.x + ' ' + source7.y + ' H ' + trunkX + ' M ' + trunkX + ' ' + targetY + ' H ' + targetX);
                }
                if (source8) {
                    root.querySelector('#chart-line-8').setAttribute('d', 'M ' + source8.x + ' ' + source8.y + ' H ' + trunkX);
                }
                if (source9) {
                    root.querySelector('#chart-line-9').setAttribute('d', 'M ' + source9.x + ' ' + source9.y + ' H ' + trunkX);
                }

                // 5번 버튼 우측 중심 -> 평가·관리 배지 좌측 중심 (주황색 선분)
                var source5 = root.querySelector('#chart-item-5');
                var evalBadge = root.querySelector('#chart-blue-eval');
                var line5 = root.querySelector('#chart-line-5');
                if (source5 && evalBadge && line5) {
                    var source5Rect = source5.getBoundingClientRect();
                    var evalRect = evalBadge.getBoundingClientRect();
                    var x5 = source5Rect.right - rowRect.left;
                    var y5 = source5Rect.top + source5Rect.height / 2 - rowRect.top;
                    var evalX = evalRect.left - rowRect.left;
                    var evalY = evalRect.top + evalRect.height / 2 - rowRect.top;
                    var midX = x5 + Math.max(4, (evalX - x5) / 2);
                    line5.setAttribute('d', 'M ' + x5 + ' ' + y5 + ' H ' + midX + ' V ' + evalY + ' H ' + evalX);
                }

                // 녹색 섹션: 2·4번 버튼 우측 중심 -> 가장 왼쪽 오브젝트(점검 배지)의 좌측 중심 (녹색 선분)
                var greenRow = root.querySelector('.chart-section-green .chart-row-flex');
                var greenSvg = root.querySelector('.chart-section-green .chart-connectors-green');
                var greenBadge = root.querySelector('#chart-green-check');
                if (greenRow && greenSvg && greenBadge) {
                    var gRowRect = greenRow.getBoundingClientRect();
                    var gBadgeRect = greenBadge.getBoundingClientRect();
                    var gWidth = Math.max(1, Math.ceil(gRowRect.width));
                    var gHeight = Math.max(1, Math.ceil(gRowRect.height));
                    greenSvg.setAttribute('viewBox', '0 0 ' + gWidth + ' ' + gHeight);

                    var gSources = {};
                    ['g2', 'g4'].forEach(function (key) {
                        var src = root.querySelector('#chart-item-' + key);
                        if (!src) return;
                        var srcRect = src.getBoundingClientRect();
                        gSources[key] = {
                            x: srcRect.right - gRowRect.left,
                            y: srcRect.top + srcRect.height / 2 - gRowRect.top,
                        };
                    });

                    var gTargetX = gBadgeRect.left - gRowRect.left;
                    var gTargetY = gBadgeRect.top + gBadgeRect.height / 2 - gRowRect.top;
                    var gTrunkX = gTargetX - 8;

                    if (gSources.g2) {
                        root.querySelector('#chart-line-g2').setAttribute('d', 'M ' + gSources.g2.x + ' ' + gSources.g2.y + ' H ' + gTrunkX + ' V ' + gTargetY);
                    }
                    if (gSources.g4) {
                        root.querySelector('#chart-line-g4').setAttribute('d', 'M ' + gSources.g4.x + ' ' + gSources.g4.y + ' H ' + gTrunkX + ' V ' + gTargetY + ' H ' + gTargetX);
                    }
                }
            }
            ...
            // SVG 연결선은 컨테이너 내부 절대좌표로 계산되므로, 크기/스크롤 변화 후에도
            // 즉시 다시 계산해 선분만 떠다니지 않도록 고정한다.
            var scheduleChartConnectorUpdate = (function () {
                var rafId = null;
                return function () {
                    if (rafId !== null) {
                    ...
                    rafId = requestAnimationFrame(function () {
                        rafId = null;
                        updateChartConnectors(document);
                    });
                };
            })();

            updateChartConnectors(document);
            window.addEventListener('resize', scheduleChartConnectorUpdate);
            window.addEventListener('scroll', scheduleChartConnectorUpdate, { passive: true });
            if (document.fonts && document.fonts.ready) {
                document.fonts.ready.then(scheduleChartConnectorUpdate);
            }
            if (typeof ResizeObserver !== 'undefined') {
                var chartResizeObserver = new ResizeObserver(scheduleChartConnectorUpdate);
                var chartRowForObserver = document.querySelector('.chart-section-blue .chart-row-flex');
                if (chartRowForObserver) {
                    chartResizeObserver.observe(chartRowForObserver);
                }
            }
            // 스무스 스크롤 완료 시점을 고려해 한 번 더 재계산
            setTimeout(scheduleChartConnectorUpdate, 300);
            setTimeout(scheduleChartConnectorUpdate, 1000);
            ...
            function setupClauseEditorModal() {
                ...
                var insertPositionModal = document.getElementById('insert-position-modal');
                var insertPositionSelect = document.getElementById('insert-position-select');
                var insertPositionClose = document.getElementById('insert-position-close');
                var insertPositionCancel = document.getElementById('insert-position-cancel');
                var insertPositionConfirm = document.getElementById('insert-position-confirm');
                ...
                if (!modal || !closeBtn || !cancelBtn || !deleteBtn || !saveBtn || !textarea || !originalTextarea || !insertAfterSelect || !basisTextarea || !modalTitle || !insertAfterGroup) {
                    return;
                }
                ...
                function closeInsertPositionModal() {
                    if (!insertPositionModal) return;
                    insertPositionModal.classList.remove('is-open');
                    insertPositionModal.setAttribute('aria-hidden', 'true');
                }

                function openInsertPositionModal() {
                    if (!insertPositionModal || !insertPositionSelect) return;
                    ...
                    articleNodes.forEach(function (node, index) {
                        ...
                        option.textContent = normalizeHeadingText(node.textContent || '') + ' 뒤에 추가';
                        insertPositionSelect.appendChild(option);
                    });
                    if (articleNodes.length === 0) {
                        alert('추가할 조항 위치가 없습니다.');
                        return;
                    }
                    insertPositionModal.classList.add('is-open');
                    insertPositionModal.setAttribute('aria-hidden', 'false');
                }
                ...
                function replaceSubtitleClause(clause, draft) {
                    var wrapper = clause.heading.closest('.rule-table-wrap');
                    if (!wrapper || !wrapper.parentNode) return;
                    clause.heading.textContent = draft.headingText;
                    (clause.nodes || []).slice(1).forEach(function (node) {
                        if (node && node.parentNode) node.parentNode.removeChild(node);
                    });
                    var insertBeforeNode = wrapper.nextElementSibling;
                    draft.bodyLines.forEach(function (line) {
                        var paragraph = document.createElement('p');
                        paragraph.textContent = line;
                        wrapper.parentNode.insertBefore(paragraph, insertBeforeNode);
                    });
                }
                ...
                if (insertPositionClose) insertPositionClose.addEventListener('click', closeInsertPositionModal);
                if (insertPositionCancel) insertPositionCancel.addEventListener('click', closeInsertPositionModal);
                if (insertPositionConfirm) insertPositionConfirm.addEventListener('click', function () {
                    var articleNodes = getArticleHeadingNodes();
                    var index = parseInt(insertPositionSelect.value || '-1', 10);
                    var targetHeading = articleNodes[index];
                    if (!targetHeading) return;
                    closeInsertPositionModal();
                    openModal({ mode: 'insert', heading: targetHeading, nodes: [{ textContent: '' }] });
                    textarea.value = '';
                    originalTextarea.value = '';
                    basisTextarea.value = '';
                    populateInsertAfterOptions(targetHeading);
                });
                ...
                saveBtn.addEventListener('click', function () {
                    ...
                    persistModalMutation(saveBtn, '\uC800\uC7A5 \uC911...', function () {
                        if (editingClause.isSubtitle) {
                            replaceSubtitleClause(editingClause, draft);
                        } else {
            ...
            function buildPrintPreview() {
                ...
                function startChartPage() {
                    var chartSource = document.querySelector('#law-tab-chart .chart-frame');
                    if (!chartSource) {
                        return;
                    }

                    var chartPage = document.createElement('section');
                    chartPage.className = 'print-preview-page print-chart-page';
                    if (sampleHeaderTable) {
                        chartPage.appendChild(sampleHeaderTable.cloneNode(true));
                    }

                    var chartDocument = document.createElement('div');
                    chartDocument.className = 'document';
                    chartDocument.style.overflow = 'hidden';
                    var chartClone = chartSource.cloneNode(true);
                    chartClone.classList.add('print-chart-frame');
                    var chartTitle = document.createElement('div');
                    chartTitle.className = 'print-chart-title';
                    ['안전 및 보건 확보 의무', '이행 체계도'].forEach(function (line) {
                        var titleLine = document.createElement('span');
                        titleLine.textContent = line;
                        titleLine.style.whiteSpace = 'nowrap';
                        chartTitle.appendChild(titleLine);
                    });
                    chartClone.querySelector('.chart-section-blue').appendChild(chartTitle);
                    chartClone.querySelectorAll('button').forEach(function (button) {
                    ...
                    chartDocument.appendChild(chartClone);
                    chartPage.appendChild(chartDocument);
                    printPreviewPaper.appendChild(chartPage);

                    // A4 한 페이지에 들어가도록 차트를 자동 축소
                    var chartPageStyle = window.getComputedStyle(chartPage);
                    var chartPageVPad = parseFloat(chartPageStyle.paddingTop || '0') + parseFloat(chartPageStyle.paddingBottom || '0');
                    var chartPageHeader = chartPage.querySelector('.rule-table-wrap');
                    var chartPageHeaderH = chartPageHeader ? chartPageHeader.offsetHeight : 0;
                    // 하단 잘림 방지를 위해 문서 패딩과 안전 여백(약 8mm)을 추가로 제외
                    var chartDocStyle = window.getComputedStyle(chartDocument);
                    var chartDocVPad = parseFloat(chartDocStyle.paddingTop || '0') + parseFloat(chartDocStyle.paddingBottom || '0');
                    var chartSafetyMargin = 70;
                    var chartAvailH = Math.max(0, chartPage.clientHeight - chartPageVPad - chartPageHeaderH - chartDocVPad - chartSafetyMargin);
                    var chartAvailW = chartDocument.clientWidth;

                    var naturalH = Math.max(chartClone.scrollHeight, chartClone.getBoundingClientRect().height);
                    var naturalW = Math.max(chartClone.scrollWidth, chartClone.getBoundingClientRect().width);
                    // 가용 높이 전체를 사용해 최대한 확대 (안전 여백은 별도 확보됨)
                    var scale = Math.min(1, chartAvailH / naturalH, chartAvailW / naturalW);
                    if (scale < 1) {
                        chartClone.style.transform = 'scale(' + scale + ')';
                        chartClone.style.transformOrigin = 'top left';
                    }

                    updateChartConnectors(chartDocument);
                    chartDocument.style.height = Math.ceil(naturalH * scale + 12) + 'px';
