...
TRANSACTION_DATETIME_PATTERN = re.compile(
    r"(?P<yy>\d{2})\s*[/.\-]\s*(?P<mm>\d{2})\s*[/.\-]\s*(?P<dd>\d{2})"
    r"\s+"
    r"(?P<hh>\d{2})\s*[:.]\s*(?P<mi>\d{2})\s*[:.]\s*(?P<ss>\d{2})"
)
...
def detect_receipt_quad(image: np.ndarray, min_area_ratio: float = 0.08, max_candidates: int = 12) -> Optional[np.ndarray]:
    ...
    for cnt in contours[: max_candidates * 5]:
    ...
    # Fallback: use minAreaRect on large contours if 4-point contour is not found.
    if not candidates:
        for cnt in contours[: max_candidates * 5]:
            area = cv2.contourArea(cnt)
            if area < img_area * min_area_ratio:
    ...
    # Fallback 2: threshold-based document detection on bright paper-like region.
    if not candidates:
        ...
        for cnt in contours2[: max_candidates * 3]:
            area = cv2.contourArea(cnt)
            if area < img_area * min_area_ratio:
                continue
            rect = cv2.minAreaRect(cnt)
    ...
    if content_quad is not None:
        is_duplicate = any(
            np.allclose(order_points(existing), order_points(content_quad), atol=8.0)
    ...
    for quad in candidates:
        s = score_candidate_quad(image, quad)
        if s > best_score:
            best_score = s
            best_quad = quad
...
def crop_receipt(image: np.ndarray, options: CropOptions) -> Tuple[np.ndarray, bool]:
    ...
    if options.margin > 0:
        quad = expand_quad_from_center(quad, options.margin, image.shape)

    warped = four_point_transform(image, quad)

    if options.auto_rotate_vertical and warped.shape[1] > warped.shape[0]:
...
def generate_datetime_ocr_variants(image: np.ndarray) -> List[np.ndarray]:
    ...
    return [up2_th, gray, adapt, top_up2_th]
...
def parse_transaction_datetime_candidate(text: str) -> Optional[datetime]:
    ...
    match = TRANSACTION_DATETIME_PATTERN.search(normalized)
    if match is None:
        return None
...
class ReceiptBatchCropperApp:
    ...
    def _build_ui(self):
        ...
        help_text = (
            "- 사진 여러 장을 넣으면 영수증 영역을 찾아 자동으로 잘라냅니다.\n"
            "- 저장 파일명은 거래일시를 읽어 YYYYMMDD_HHMMSS.jpg 형태로 만듭니다.\n"
            "- OCR로 거래일시를 못 읽으면 원본 파일명_crop 으로 저장합니다.\n"
            "- 배경이 복잡하거나 그림자가 심하면 일부 파일은 실패할 수 있습니다.\n"
            "- 실패 파일은 원본 그대로 _fail 이름으로 저장할 수 있습니다.\n"
            "- 첫 버전은 영수증 1장 기준으로 동작합니다.\n"
            "- 여러 영수증이 한 사진에 같이 있으면 원하는 대로 안 잘릴 수 있습니다."
        )
        ...
        ttk.Button(status_row, text="실행", command=self.start_processing).pack(side="right")
    ...
    def start_processing(self):
        ...
        threading.Thread(target=self.process_batch, args=(self.files.copy(), out_dir, options), daemon=True).start()
    ...
    def process_batch(self, files: List[str], out_dir: str, options: CropOptions):
        ...
        for i, path in enumerate(files, start=1):
            self.status_var.set(f"처리 중: {i}/{total} - {os.path.basename(path)}")
            try:
                img = cv2.imdecode(np.fromfile(path, dtype=np.uint8), cv2.IMREAD_COLOR)
                if img is None:
                    raise ValueError("이미지를 읽을 수 없습니다.")

                cropped, ok = crop_receipt(img, options)
                ...
                if ok:
                    ...
                    if transaction_dt is not None:
                        self.log(
                            f"[성공] {os.path.basename(path)} -> {os.path.basename(save_path)} "
                            f"(거래일시: {transaction_dt.strftime('%Y-%m-%d %H:%M:%S')})"
                        )
                    else:
                        self.log(
                            f"[성공] {os.path.basename(path)} -> {os.path.basename(save_path)} "
                            "(거래일시 OCR 실패, 기본 파일명 사용)"
                        )
                else:
                    failed += 1
                    if options.save_failures_copy:
                        ...
                        self.log(f"[실패] {os.path.basename(path)} -> {os.path.basename(fail_path)} (원본 저장)")
                    else:
